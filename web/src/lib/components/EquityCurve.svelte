<script lang="ts">
  ...
  /** Which series to draw: 'nav' (per-share NAV — reflects real performance, unaffected by deposits) or 'equity' (raw pool USDT — visible deposit/withdraw steps). */
  export let mode: 'nav' | 'equity' = 'nav';
  export let height = 220;

  $: data = points.map((p) => ({
    x: p.taken_at,
    y: mode === 'nav' ? p.nav : p.total_equity,
    src: p.source
  }));
...
<div class="card p-5">
  <div class="flex items-start justify-between gap-4 mb-4">
    <div>
      <div class="label">{mode === 'nav' ? 'NAV / 每份额价值' : '基金总资金（USDT）'}</div>
      <div class="flex items-baseline gap-3 mt-1">
        <div class="stat-value">
          {mode === 'nav' ? last.toFixed(6) : fmtUSDT(last, 2)}
        </div>
        {#if hasData}
  ...
  {:else}
    ...
    {#if hover}
      <div class="mt-2 text-xs font-mono text-ink-300 flex justify-between">
        ...
        <span class="text-ink-100">{mode === 'nav' ? hover.pt.y.toFixed(6) : fmtUSDT(hover.pt.y, 2) + ' USDT'}</span>
      </div>
    {/if}
  {/if}
</div>
